...
export default function WeveDesignLanding({
  ...
  const methodMobileSummary =
    selectedConstructionModel.id === 'cm'
      ? '원하는 디자인을 직접 고르고, 위브디자인이 견적과 현장 흐름을 관리하는 방식입니다.'
      : '디자인 제안부터 견적, 시공, 마감 확인까지 한 흐름으로 맡기는 방식입니다.';
  const methodSupportNote =
    selectedConstructionModel.id === 'cm'
      ? {
          title: '예산 메모',
          body: '고객이 디자인 선택과 일부 의사결정을 직접 가져가면, 턴키 방식보다 총 공사 금액을 조정할 여지가 있습니다.',
        }
      : {
          title: '관리 메모',
          body: '디자인, 자재, 공정 결정을 한 흐름으로 묶어 일정과 책임 주체를 명확하게 관리합니다.',
        };
  const methodMobileInfoSections = [
    { title: '고객 준비사항', items: selectedConstructionModel.customerRole.slice(0, 3), Icon: Check, iconClass: 'text-[#8f6f43]' },
    { title: '장점', items: selectedConstructionModel.pros.slice(0, 3), Icon: Check, iconClass: 'text-[#2f7d45]' },
    { title: '확인할 점', items: selectedConstructionModel.cons, Icon: Sparkles, iconClass: 'text-[#8f6f43]' },
  ];
          ...
          <div className="mx-auto max-w-6xl">
            <div className="grid min-w-0 gap-5 lg:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[300px_minmax(0,1fr)]">
              <div className="method-sticky-panel min-w-0 max-w-full rounded-lg border border-[#eadfcd] bg-white/86 p-5 backdrop-blur lg:sticky lg:top-28 lg:self-start">
                ...
                <div className="mt-5 grid grid-cols-2 gap-2 sm:mt-6 sm:grid-cols-1 sm:gap-3">
                  {constructionModels.map((model) => (
                    ...
                    >
                      <span className="method-mode-eyebrow hidden text-xs font-bold uppercase tracking-[0.2em] opacity-65 sm:block">{model.eyebrow}</span>
                      <span className="flex items-center justify-between gap-2 text-base font-semibold sm:mt-2 sm:gap-3 sm:text-lg">
                        {model.label}
                        <ArrowRight className="transition group-hover:translate-x-1" size={18} />
                      </span>
                      <span className="mt-3 hidden text-sm leading-6 opacity-80 sm:block">{model.bestFor}</span>
                    </button>
                  ))}
                </div>

                <div className="mt-5 hidden rounded-lg border border-[#eadfcd] bg-[#fffaf0] p-4 md:block">
              ...
              <div className="grid min-w-0 max-w-full gap-5 overflow-hidden">
                <div className="method-focus-card min-w-0 overflow-hidden rounded-lg border border-[#eadfcd] bg-white text-[#171512] shadow-[0_24px_70px_rgba(57,46,31,0.10)]">
                  ...
                  <div className="grid gap-2 border-t border-[#eadfcd] bg-[#fffdf8] p-4 md:hidden">
                    {methodMobileInfoSections.map(({ title, items, Icon, iconClass }) => (
                      <details key={title} className="group rounded-md border border-[#eadfcd] bg-white">
                        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 marker:content-none">
                          <span className="text-sm font-bold tracking-[0.12em] text-[#8f6f43]">{title}</span>
                          <span className="text-lg leading-none text-[#8f6f43] transition group-open:rotate-45">+</span>
                        </summary>
                        <ul className="grid gap-2 px-4 pb-4 text-sm leading-6 text-[#625d54]">
                          {items.map((item) => (
                            <li key={item} className="flex gap-2">
                              <Icon className={`mt-1 shrink-0 ${iconClass}`} size={15} />
                              {item}
                            </li>
                          ))}
                        </ul>
                ...
                <div className="min-w-0 max-w-full overflow-hidden rounded-lg border border-[#eadfcd] bg-white p-4 shadow-[0_18px_55px_rgba(57,46,31,0.07)] md:p-6">
                  <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
                    ...
                    <p className="hidden max-w-md text-sm leading-6 text-[#625d54] md:block">
